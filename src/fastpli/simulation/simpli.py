...
class Simpli(__Simpli):
    ...
    def generate_tissue(self, only_tissue=False):
        ...
        fbs_properties = [lys.data() for lys in self._fiber_bundles.layers]
        self.__gen.set_fiber_bundles(self._fiber_bundles.as_array(),
                                     fbs_properties)
        ...
        if not np.any(tissue):
            warnings.warn(
                ...
                'no fiber_bundles or that the voxel_size is to large.',
                UserWarning)
    ...
    def _check_generation_input(self):
        ...
        # if any(self._fiber_bundles.layers is None):
        for lys in self._fiber_bundles.layers:
            for ly in lys:
                if ly is None:
                    raise ValueError('fiber_bundle properties are \"None\"')
