...
with h5py.File(os.path.join(FILE_PATH, 'ref_new.h5'), 'w') as h5f:
    ...
    h5f['data/0'] = simpli.run_simulation(label_field, vec_field,
                                          tissue_properties, 0, 0)

    print("run_simulation: 1")
    h5f['data/1'] = simpli.run_simulation(label_field, vec_field,
                                          tissue_properties, np.deg2rad(5.5),
                                          np.deg2rad(0))

    print("run_simulation: 2")
    h5f['data/2'] = simpli.run_simulation(label_field, vec_field,
                                          tissue_properties, np.deg2rad(5.5),
                                          np.deg2rad(90))

    print("run_simulation: 3")
    h5f['data/3'] = simpli.run_simulation(label_field, vec_field,
                                          tissue_properties, np.deg2rad(5.5),
                                          np.deg2rad(180))

    print("run_simulation: 4")
    h5f['data/4'] = simpli.run_simulation(label_field, vec_field,
                                          tissue_properties, np.deg2rad(5.5),
                                          np.deg2rad(270))

os.system('h5diff ' + os.path.join(FILE_PATH, 'ref_new.h5') + ' ' +
          os.path.join(FILE_PATH, 'ref_68ae085.h5'))
